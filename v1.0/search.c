...

{
  ...
  for (i = 0; i < gop_conn[op].num_E; i++)
  {
    ...
    for (j = 0; j < gef_conn[ef].num_f_PC; j++)
    {
      ...
      if (!number_comparison_holds(comp, source_val, val))
        break;
    }
    if (j < gef_conn[ef].num_f_PC)
      continue;
