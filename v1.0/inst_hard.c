...

{
  ...
  case ATOM:
    ...
    {
      h = (w->fact->args[j] < 0) ? linst_table[DECODE_VAR(w->fact->args[j])] : w->fact->args[j];
      if (h < 0)
      {
        ok = FALSE;
...

{
  ...
  case FHEAD:
    ...
    {
      h = ((*n)->fluent->args[j] < 0) ? linst_table[DECODE_VAR((*n)->fluent->args[j])] : (*n)->fluent->args[j];
      if (h < 0)
      {
        ok = FALSE;
      }
...

{
  ...
  for (l = ll; l; l = l->next)
  {
    if (l->negated)
    {
      ...
      for (i = 0; i < garity[l->fact.predicate]; i++)
      {
        e->dels[e->num_dels].args[i] = (l->fact.args[i] < 0) ? linst_table[DECODE_VAR(l->fact.args[i])] : l->fact.args[i];
      }
      e->num_dels++;
    }
    else
    {
      ...
      for (i = 0; i < garity[l->fact.predicate]; i++)
      {
        e->adds[e->num_adds].args[i] = (l->fact.args[i] < 0) ? linst_table[DECODE_VAR(l->fact.args[i])] : l->fact.args[i];
      }
      e->num_adds++;
    }
  }
...

{
  ...
  for (n = ne; n; n = n->next)
  {
    ...
    {
      e->numeric_effects_fluent[m].args[i] = (n->fluent.args[i] < 0) ? linst_table[DECODE_VAR(n->fluent.args[i])] : n->fluent.args[i];
    }
