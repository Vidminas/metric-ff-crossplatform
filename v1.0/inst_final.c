  ...
  for (a = gactions; a; a = a->next)
  {
    if (a->norm_operator)
    {
      ...
      for (ne = no->effects; ne; ne = ne->next)
      {
        ...
        {
          if (ne->numeric_effects_neft[i] != ASSIGN)
            continue;
          lf = ne->numeric_effects_fluent[i].function;
          for (j = 0; j < gf_arity[lf]; j++)
          {
            lf_args[j] = (ne->numeric_effects_fluent[i].args[j] >= 0) ? ne->numeric_effects_fluent[i].args[j] : a->inst_table[DECODE_VAR(ne->numeric_effects_fluent[i].args[j])];
          }
          adr = fluent_adress();
          if (!lf_def[lf][adr])
          {
    ...
    else
    {
      ...
      for (pae = pa->effects; pae; pae = pae->next)
      {
        ...
        {
          if (pae->numeric_effects_neft[i] != ASSIGN)
            continue;
          lf = pae->numeric_effects_fluent[i].function;
          for (j = 0; j < gf_arity[lf]; j++)
          {
            lf_args[j] = (pae->numeric_effects_fluent[i].args[j] >= 0) ? pae->numeric_effects_fluent[i].args[j] : a->inst_table[DECODE_VAR(pae->numeric_effects_fluent[i].args[j])];
          }
          adr = fluent_adress();
          if (!lf_def[lf][adr])
          {
...

{
  ...
  {
  case AD:
    if (!set_relevants_in_exp(&((*n)->leftson)))
      return FALSE;
    if (!set_relevants_in_exp(&((*n)->rightson)))
      return FALSE;
  ...
  case DI:
    ...
    break;
  case MINUS:
    if (!set_relevants_in_exp(&((*n)->son)))
      return FALSE;
    if ((*n)->son->connective != NUMBER)
      break;
    (*n)->connective = NUMBER;
  ...
  while (a)
  {
    if (a->norm_operator)
    {
      ...
      for (i = 0; i < no->num_numeric_preconds; i++)
      {
        ...
        if (!set_relevants_in_exp(&(a->numeric_preconds_lh[a->num_numeric_preconds])))
          break;
        ...
        if (!set_relevants_in_exp(&(a->numeric_preconds_rh[a->num_numeric_preconds])))
          break;
      ...
      for (ne = no->effects; ne; ne = ne->next)
      {
        ...
        for (i = 0; i < ne->num_conditions; i++)
        {
          ...
          for (j = 0; j < garity[lp]; j++)
          {
            largs[j] = (ne->conditions[i].args[j] >= 0) ? ne->conditions[i].args[j] : a->inst_table[DECODE_VAR(ne->conditions[i].args[j])];
          }
          adr = fact_adress();
          if (!lpos[lp][adr])
        ...
        for (i = 0; i < ne->num_numeric_conditions; i++)
        {
          ...
          if (!set_relevants_in_exp(&(aa->numeric_conditions_lh[aa->num_numeric_conditions])))
            break;
          ...
          if (!set_relevants_in_exp(&(aa->numeric_conditions_rh[aa->num_numeric_conditions])))
            break;
        ...
        for (i = 0; i < ne->num_adds; i++)
        {
          ...
          for (j = 0; j < garity[lp]; j++)
          {
            largs[j] = (ne->adds[i].args[j] >= 0) ? ne->adds[i].args[j] : a->inst_table[DECODE_VAR(ne->adds[i].args[j])];
          }
          adr = fact_adress();
          if (!lneg[lp][adr])
...

{
  ...
  case FHEAD:
    ...
    {
      h = ((*n)->fluent->args[j] < 0) ? a->inst_table[DECODE_VAR((*n)->fluent->args[j])] : (*n)->fluent->args[j];
      if (h < 0)
      {
        ok = FALSE;
      }
...

{
  ...
  for (a = gactions; a; a = a->next)
  {
    ...
    for (i = 0; i < a->num_effects; i++)
    {
      ...
      for (j = 0; j < a->num_preconds; j++)
      {
        ...
        {
          if (gef_conn[n_ef].PC[k] == a->preconds[j])
            break;
        }
      ...
      for (j = 0; j < e->num_conditions; j++)
      {
        for (k = 0; k < gef_conn[n_ef].num_PC; k++)
        {
          if (gef_conn[n_ef].PC[k] == e->conditions[j])
            break;
        }
        if (k < gef_conn[n_ef].num_PC)
          continue;
      ...
      for (j = 0; j < a->num_lnf_preconds; j++)
      {
        ...
        {
          if (gef_conn[n_ef].f_PC_fl[k] == a->lnf_preconds_lh[j]->pF[0])
            break;
        }
      ...
      for (j = 0; j < e->num_lnf_conditions; j++)
      {
        ...
        for (k = 0; k < gef_conn[n_ef].num_f_PC; k++)
        {
          if (gef_conn[n_ef].f_PC_fl[k] == e->lnf_conditions_lh[j]->pF[0])
            break;
        }
        if (k < gef_conn[n_ef].num_f_PC)
        {
      ...
      for (j = 0; j < e->num_adds; j++)
      {
        for (k = 0; k < gef_conn[n_ef].num_PC; k++)
        {
          if (gef_conn[n_ef].PC[k] == e->adds[j])
            break;
        }
        if (k < gef_conn[n_ef].num_PC)
          continue;
      ...
      for (i = 0; i < gop_conn[n_op].num_E - 1; i++)
      {
        ...
        for (j = i + 1; j < gop_conn[n_op].num_E; j++)
        {
          ...
          for (k = 0; k < gef_conn[ef_].num_PC; k++)
          {
            ...
            {
              if (gef_conn[ef].PC[l] == gef_conn[ef_].PC[k])
                break;
            }
            if (l == gef_conn[ef].num_PC)
              break;
          }
            ...
            for (k = 0; k < gnum_fl_conn; k++)
            {
              ...
              if (gef_conn[ef].f_PC_direct_comp[k] == IGUAL ||
                  ...
                  (gef_conn[ef].f_PC_direct_c[k] == gef_conn[ef_].f_PC_direct_c[k] &&
                   ...
                   gef_conn[ef_].f_PC_direct_comp[k] == GE))
                break;
            }
            if (k == gnum_fl_conn)
            {
          ...
          for (k = 0; k < gef_conn[ef].num_PC; k++)
          {
            ...
            {
              if (gef_conn[ef_].PC[l] == gef_conn[ef].PC[k])
                break;
            }
            if (l == gef_conn[ef_].num_PC)
              break;
          }
            ...
            for (k = 0; k < gnum_fl_conn; k++)
            {
              ...
              if (gef_conn[ef_].f_PC_direct_comp[k] == IGUAL ||
                  ...
                  (gef_conn[ef_].f_PC_direct_c[k] == gef_conn[ef].f_PC_direct_c[k] &&
                   ...
                   gef_conn[ef].f_PC_direct_comp[k] == GE))
                break;
            }
            if (k == gnum_fl_conn)
            {
  ...
  for (j = 0; j < gnum_logic_goal; j++)
  {
    ...
    {
      if (gflogic_goal[k] == glogic_goal[j])
        break;
    }
    if (k < gnum_flogic_goal)
      continue;
  ...
  for (j = 0; j < gnum_lnf_goal; j++)
  {
    ...
    {
      if (gfnumeric_goal_fl[k] == glnf_goal_lh[j]->pF[0])
        break;
    }
  ...
  if (gcmd_line.display_info == 125)
  {
    ...
    for (i = 0; i < gnum_ft_conn; i++)
    {
      ...
      for (j = 0; j < gft_conn[i].num_PC; j++)
      {
        ...
        printf(" - op ");
        print_op_name(gef_conn[gft_conn[i].PC[j]].op);
      }
      printf("\n----------ADD BY:");
      for (j = 0; j < gft_conn[i].num_A; j++)
      {
        ...
        printf(" - op ");
        print_op_name(gef_conn[gft_conn[i].A[j]].op);
      }
      printf("\n----------DEL BY:");
      for (j = 0; j < gft_conn[i].num_D; j++)
      {
        ...
        printf(" - op ");
        print_op_name(gef_conn[gft_conn[i].D[j]].op);
      }
    }
    ...
    for (i = 0; i < gnum_fl_conn; i++)
    {
      ...
      for (j = 0; j < gfl_conn[i].num_PC; j++)
      {
        ...
        printf(" - op ");
        print_op_name(gef_conn[gfl_conn[i].PC[j]].op);
      }
      printf("\n----------INCREASED BY:");
      for (j = 0; j < gfl_conn[i].num_IN; j++)
      {
        if (gfl_conn[i].IN_fl_[j] == -1)
        {
          ...
          printf("  ---  op ");
          print_op_name(gef_conn[gfl_conn[i].IN[j]].op);
        }
        else
        {
      ...
      for (j = 0; j < gfl_conn[i].num_AS; j++)
      {
        if (gfl_conn[i].AS_fl_[j] == -1)
        {
          ...
          printf("  ---  op ");
          print_op_name(gef_conn[gfl_conn[i].AS[j]].op);
        }
        else
        {
